...
async function main() {
  ...
  const server=http.createServer(async(req,res)=>{
    ...
    try {
      ...
      res.end(type==='LEDGER' ? response.replace(/<CLOSINGBALANCE TYPE="String">[^<]*<\/CLOSINGBALANCE>/,'<CLOSINGBALANCE TYPE="Amount"/>')
        :type==='VOUCHER'?response.replace(/<AMOUNT TYPE="String">[^<]*<\/AMOUNT>/,'<AMOUNT TYPE="Amount">USD 1,23,456.789 Cr</AMOUNT>'):response);
  ...
  try {
    ...
    assert.equal(manifest.collections.length,13);assert.ok(manifest.collections.every(c=>c.status==='success'));
