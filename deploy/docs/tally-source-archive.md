...
Dashboard sync validates the latest complete snapshot per company before applying interpreted LEDGER / VOUCHER / COSTCENTRE data to reporting tables. Company identity comes from snapshot metadata. Missing voucher amounts, absent ledger closing balances and rejected records block the company reload; opening balances never substitute for closing balances. Explicit empty Tally closing-balance fields keep their zero interpretation, and cancelled/optional vouchers are excluded. Raw JSONB remains untouched. Valid snapshots update changed records and remove absent Tally-sourced rows under the existing full-snapshot rules.

Source completion commits the archive, then automatically triggers sync for complete captures. Archive success and reporting validation have independent statuses; a sync failure does not fail or roll back a saved export. Missing business fields are preserved by export and assessed by sync. Partial captures remain archived but cannot replace reporting data.

Retry sync from Operations / Sync Tally, `npm run promote` in `api`, or `POST /api/tally/sync` while signed in. `GET /api/tally/sync` returns per-company progress and validation failures. Reprocessing an applied complete snapshot validates it again, and forced replay cannot overwrite a newer snapshot.
