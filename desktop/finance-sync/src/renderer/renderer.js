...
function resultDescription(row){
  ...
  if(row.coverageStatus==='partial')return 'Partial capture saved. Dashboard data was not replaced. Check Tally and sync again.';
...
function render(next){
  ...
  $('last-sync').textContent=date(state.lastSuccessfulSync);$('pending').textContent=`${state.pending.count} pending`;
  ...
  $('pending-note').textContent=state.pending.damaged?'A saved upload needs administrator attention. Export a diagnostic log before making changes.':missing.length?'Some saved uploads belong to companies that are not currently available. Load those companies in Tally to retry them.':'Saved captures for checked companies will be retried first. Those companies will get a fresh capture on the following run.';
  $('check').disabled=state.busy;$('check').textContent=state.phase==='idle'?'Check connection':'Check again';
  $('stop').hidden=!state.busy;$('stop').disabled=state.phase==='stopping';$('stop').textContent=state.phase==='checking'?'Stop checking':'Stop sync';
...
$('sync').addEventListener('click',()=>invoke('sync',[...selection]));
...
invoke('snapshot').then(result=>{if(result.ok){render(result.value);if(result.value.phase==='idle')invoke('check');}});
